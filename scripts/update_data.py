...
def process(xl):
    ...
    for _,r in data.iterrows():
        ...
        weekly.append([
            clean(r.iloc[2]),wk,show,clean(r.iloc[4]),clean(r.iloc[5]),
...
def main():
    ...
    new_block = (
        f"// ══ DATA — Auto-generated {today} ══════════════════════════════════════════\n"
        f"// WDATA:{len(weekly)} rows  YDATA:{len(yearly)} rows  DDATA:{len(daily)} rows\n"
        f"const WDATA={json.dumps(weekly,ensure_ascii=False,separators=(',',':'))};\n"
        f"const YDATA={json.dumps(yearly,ensure_ascii=False,separators=(',',':'))};\n"
        f"const DDATA={json.dumps(daily, ensure_ascii=False,separators=(',',':'))};\n"
    )
    ...
    # Replace just the data arrays (between the Auto-generated comment and the DDATA line end)
    # Use regex to replace the 3 const arrays
    pattern = r'// ══ DATA.*?const DDATA=\[.*?\];'
    if not re.search(pattern, html, re.DOTALL):
        print("ERROR: Data block marker not found in index.html")
        sys.exit(1)

    new_html = re.sub(pattern, new_block.rstrip(), html, count=1, flags=re.DOTALL)

    with open(index_path,'w',encoding='utf-8') as f:
        f.write(new_html)

    print(f"✓ index.html updated ({len(new_html):,} chars)")
    print("Done. Commit & push index.html to redeploy.")
